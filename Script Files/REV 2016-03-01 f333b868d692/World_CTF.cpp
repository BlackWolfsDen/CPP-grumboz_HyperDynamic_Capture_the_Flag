...
{
	// Create the database table if not allready existing.

	WorldDatabase.PExecute("CREATE TABLE IF NOT EXISTS `grumboz_wctf_flag_gps` (`guid` BIGINT(20) UNSIGNED NOT NULL COMMENT 'guid of flag',`name` VARCHAR(255) NOT NULL, `map_id` MEDIUMINT(8) UNSIGNED NOT NULL COMMENT 'map id', `area_id` MEDIUMINT(8) UNSIGNED NOT NULL COMMENT 'area id', `zone id` MEDIUMINT(8) UNSIGNED NOT NULL COMMENT 'zone id', `x` FLOAT NOT NULL COMMENT 'x', `y` FLOAT NOT NULL COMMENT 'y', `z` FLOAT NOT NULL COMMENT 'z', `o` FLOAT NOT NULL COMMENT 'o', UNIQUE INDEX `guid` (`guid`) ) COLLATE = 'utf8_general_ci' ENGINE = InnoDB ; ");

	// Loading prestored World Flag's info from db

	uint32 flag_count = 1;

	QueryResult WorldFlagGps_Query = WorldDatabase.PQuery("SELECT * FROM grumboz_wctf_flag_gps;"); // guid, name, map_id, area_id, zone_id, x, y, z, o
...
uint32 WorldCtf::GetFlagCtfID(uint32 guid)
{
	if (sWorldCtf->WorldFlagGps.size() != 0)
	{
		...
		if (sWorldCtf->test) { TC_LOG_INFO("server.loading", "GET_FLAG_CTFID SEARCH GUID:%u", guid); }

		for (id = 1; id < sWorldCtf->WorldFlagGps.size(); id++)
		{
//			if (sWorldCtf->test) { TC_LOG_INFO("server.loading", "GET_FLAG_CTFID COMPARE GUID:%u TO TBL_GUID:%u", guid, sWorldCtf->WorldFlagGps[id].guid); }

			if (sWorldCtf->WorldFlagGps[id].guid == guid)
			{
				return id;

					if (sWorldCtf->test) { TC_LOG_INFO("server.loading", "GET_FLAG_CTFID:%u GUID:%u", id, guid); }

				break;
			}
		}

			if (sWorldCtf->test) { TC_LOG_INFO("server.loading", "GET_FLAG_CTFID:%u", id); }
	}
	return false;
}
...
void WorldCtf::UpdateWorldFlagDataBase(uint8 key, uint32 guid, std::string name, uint32 map_id, uint32 area_id, uint32 zone_id, float x, float y, float z, float o)
{
	...
	WorldDatabase.PExecute(wctf_query, guid, name.c_str(), map_id, area_id, zone_id, x, y, z, o); 
}
...
void WorldCtf::UpdateGameObject(GameObject* go)
{
	...
	uint8 activeteamid = sWorldCtf->GetActiveGO_Team_ID();
		...
		if (go->GetPhaseMask() != PHASEMASK_NORMAL) // Adjust phase mask. PHASEMASK_NORMAL = 1 PHASEMASK_ANYWHERE = -1
		{
			...
			msg = "At " + sWorldCtf->WorldFlagGps[activeteamid].name;
...
public:
	...
	std::vector<ChatCommand> GetCommands() const
	{
		static std::vector<ChatCommand> WCTFCommandTable =
		{
			...
			{ "spawn",		rbac::RBAC_IN_GRANTED_LIST, true, &HandleWCTFSpawnWorldFlagCommand, "Used to spawn World Flag's." },
		};
	...
	static bool HandleWCTFSetupCommand(ChatHandler* handler, const char* args)
	{
		...
		ChatHandler(player->GetSession()).PSendSysMessage("- Player aura check timer duration :%u seconds.", sWorldCtf->GetPlayerAuraCheckerTimer());
	...
	static bool HandleWCTFTeleCommand(ChatHandler* handler, const char* args)
	{
		...
		else
		{
			...
			{
				uint32 id = 1;
				uint32 max_id = sWorldCtf->WorldFlagGps.size();
				...
				{
					ChatHandler(player->GetSession()).PSendSysMessage("available id range 1 to %u.", max_id);
				}
