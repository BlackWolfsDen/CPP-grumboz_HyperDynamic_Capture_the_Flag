...
void WorldCtf::AddNewWorldFlag(Player* player)
{
	...
	int startphasemask = PHASEMASK_NORMAL; // player->GetPhaseMask(); // PHASEMASK_NORMAL || GO_STATE_ACTIVE GO_STATE_READY
	...
	if (objectInfo)
	{
		...
		G3D::Quat rot = G3D::Matrix3::fromEulerAnglesZYX(player->GetOrientation(), 0.f, 0.f);
			...
			if (!GO->Create(guidLow, id, map, startphasemask, *player, rot, 255, GO_STATE_ACTIVE)) // attempt to create a GO with the guidlow, id and fill the shell with data retrieved with id.
			{
		...
		map->AddToMap(GO); // visual responce of spawning flag.
			...
			if (GO->isSpawned())
			{
				...
				sWorldCtf->UpdateWorldFlagDataBase(0, GO->GetSpawnId(), map->GetMapName(), player->GetMapId(), player->GetAreaId(), player->GetZoneId(), x, y, z, o); // player->GetMapAreaAndZoneString()
			}
	}
}
			...
			World_Flag(GameObject* go) : GameObjectAI(go) { } 
				...
				void Reset() override // Called when a go just spawns or resets in general
				{
					uint32 guid = go->GetSpawnId();
						...
						if (!sWorldCtf->GetFlagCtfID(go->GetSpawnId()))
						{
							sWorldCtf->UpdateWorldFlagDataBase(0, go->GetSpawnId(), go->GetMap()->GetMapName(), go->GetMapId(), go->GetAreaId(), go->GetZoneId(), go->GetPositionX(), go->GetPositionY(), go->GetPositionZ(), go->GetOrientation());
						}
				}
...
public: WCTF_Player_Actions() : PlayerScript("WCTF_Player_Actions") { };
		...
		virtual void OnLogin(Player* player, bool firstLogin)
		{
			...
			WorldCtfPlayerInfo_Elements& data = sWorldCtf->WorldCtfPlayerInfo[guid];
			data.aura = false;
			data.captures = 0;
		}
